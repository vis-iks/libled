...
struct KerningCharsHasher {
  std::size_t operator()(const KerningChars &k) const {
    return ((std::hash<uint>()(k.first) ^ (std::hash<uint>()(k.second) << 1)) >>
            1);
  }
};
...
public:
  ...
  // Helper to find the texture filename from the font file without loading the
  // font
  ...
  // Loads a TTF font from file and generates a font texture
  // fontSize: height in pixels
  // textureWidth/Height: size of the generated texture atlas
  static Font *LoadFromTTF(const String &filename, float fontSize,
                           int textureWidth = 512, int textureHeight = 512);

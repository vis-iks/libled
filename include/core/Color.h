...
public:
    ...
    static Color Gradient(Color start, Color end, float t)
    {
        t = std::clamp(t, 0.0f, 1.0f);
        return Color(
            static_cast<byte>(start.r + (end.r - start.r) * t),
            static_cast<byte>(start.g + (end.g - start.g) * t),
            static_cast<byte>(start.b + (end.b - start.b) * t),
            static_cast<byte>(start.a + (end.a - start.a) * t)
        );
    }

    // Multiply color by float to scale intensity (preserves alpha)
    inline Color operator*(float scale) const
    {
        return Color(
            ToByte(ToFloat(r) * scale),
            ToByte(ToFloat(g) * scale),
            ToByte(ToFloat(b) * scale),
            a  // Preserve alpha
        );
    }

    // Allow float * Color as well
    friend inline Color operator*(float scale, const Color& c)
    {
        return c * scale;
    }
};

...
public:
	...
	virtual const byte* ByteData() const override final { return reinterpret_cast<const byte*>(renderbuffer.data()); }
	virtual const Color* ColorData() const override final { return renderbuffer.data(); }
	virtual MonoSampler GetMonoSampler() const override final { NOT_SUPPORTED; return MonoSampler(nullptr, 0); }
	virtual ColorSampler GetColorSampler() const override final { return ColorSampler(renderbuffer.data(), DISPLAY_WIDTH); }

	// Rasterizing methods
	void Clear(Color color);
	void CopyTo(Canvas& canvas) const {
        if(canvas.renderbuffer.size() != renderbuffer.size()) canvas.renderbuffer.resize(renderbuffer.size());
        memcpy(canvas.renderbuffer.data(), renderbuffer.data(), renderbuffer.size() * sizeof(Color)); 
    }
    ...
    void DrawColorImageMod(Point pos, const IImage& img, Color mod) { DrawColorImageMod(pos, img, mod, Rect(Point(0, 0), img.GetSize())); }

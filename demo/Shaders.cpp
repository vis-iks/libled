...
Color SineShader(float u, float v, float time) {
    ...
    float alpha = 1.0f - expDist / (maxDist * maxDist) + (1.0f - height) * -0.014f;
    
    // Color with height-based intensity
    glm::vec3 color = glm::vec3(0.9f, 0.9f, 0.9f) * height - (1.0f - alpha) * 0.652f;
    ...
    return Color(
        (byte)Clamp(color.r * 255.0f, 0.0f, 255.0f),
...
Color MagnetShader(float u, float v, float time) {
    ...
    glm::vec2 particles[] = {
        ...
        size * glm::vec2(0.3f, 0.5f),
        size * glm::vec2(0.7f, 0.5f)
    };
    ...
    float e_deriv = std::abs(std::sin(field.y * (float)e_solutions) * (float)e_solutions);
    float e_value = line_thickness - std::abs(e / glm::max(e_deriv * 0.01f, 0.001f));
    ...
    float v_deriv = std::abs(std::sin(field.x * (float)v_solutions + preview_animation) * (float)v_solutions);
    float v_value = line_thickness - std::abs(vval / glm::max(v_deriv * 0.01f, 0.001f));
    
    // Apply colors
    glm::vec3 e_color = glm::max(e_value, 0.0f) * glm::vec3(0.031f, 0.482f, 0.737f);
    glm::vec3 v_color = glm::max(v_value, 0.0f) * glm::vec3(0.031f, 0.596f, 0.490f);
    glm::vec3 color = glm::max(e_color, v_color);
    
    // Draw particles on top
    for (int i = 0; i < 3; i++) {
        glm::vec2 delta = fragCoord - particles[i];
        glm::vec3 particle_color = (charges[i] < 0) ? 
...
Color FireShader(float u, float v, float time)
{
    ...
    float n2 = FractionalBrownianMotion(glm::vec2(u * 10.0f + n1, v * 10.0f - t * 2.0f));

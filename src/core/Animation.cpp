...
String Animation::GetTextureFilename(const String& filename)
{
    TomlParser toml;
	toml.LoadFile(filename);
	return toml.GetString("Ani.Image", "");
}

Animation::Animation(const String& filename, const Image* img) :
	image(img),
	duration(0)
{
	TomlParser toml;
    ...
    // image is passed in, so we don't look it up here.

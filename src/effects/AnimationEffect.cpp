...
bool AnimatedImage::LoadGif(const String& filename)
{
    ...
    if (fread(buffer, 1, size, f) != size) {
        std::cerr << "Failed to read full GIF file: " << filename.c_str() << std::endl;
        fclose(f);
        delete[] buffer;
        return false;
    }
    ...
    int x, y, z, comp;
    // req_comp = 4 for RGBA
    unsigned char* data = stbi_load_gif_from_memory(buffer, size, &delays_data, &x, &y, &z, &comp, 4);
    
    if (!data) {
        std::cerr << "stb_image failed to load GIF: " << stbi_failure_reason() << std::endl;
        delete[] buffer;
        return false;
    }
    ...
    for (int i = 0; i < z; ++i)
    {
        ...
        // We need a helper in Image or just construct it manually via friend or new constructor
        // OR we just hack it by creating empty Image and setting fields.
        // Since Image fields are private, we need 'LoadFromMemory' or similar.
        // For MVP, assume we can add public method to Image or use subclass.
        // Wait, Image has no SetData.
        // IMPORTANT: We need to modify Image class to support setting data from memory!
    ...
    stbi_image_free(delays_data); // Wait, stbi_load_gif allocates delays array? yes doc says key is passed
    delete[] buffer;
...
{
    if (index < 0 || index >= delays.size()) return 100; // Default
    return delays[index];
}
...
AnimationEffect::AnimationEffect(std::shared_ptr<AnimatedImage> image) 
    : anim(image), currentFrame(0), lastUpdate(0), accumulator(0), playing(true)
{
}

void AnimationEffect::Render(Canvas& canvas, uint32_t timeMs)
{
    if (!anim || anim->GetFrameCount() == 0) return;
